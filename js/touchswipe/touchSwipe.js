define("touchswipe/touchSwipe","jquery",function(require, exports, module) {
	...
	var checkImgDom = function() {
		...
		imgList.each(function(index) {
			...
			if (len == 1) {
				...
				// 加载失败后 加载默认图片 加载默认图片成功一次后滑动
				
				$(this).bind('error',function(){
					this.src="wx_assets/soho/images/default.jpg";
				});
	...
	function clearAuto() {
		if (typeof _autoSlide != "undefined") {
			window.clearInterval(_autoSlide);
	...
	function argsinit() {
		...
		args = {
			...
			mCallback: function(tPoint) {
				...
				if (Math.abs(tPoint.angle) < 55) { //单点滑动角度小于15度
	...
	function WM_comUi() {
		...
		var scroll = function() {
				...
				wrapper1 = new iScroll('wrapper1', {
					disableMouse: true,
					disablePointer: true,
					vScrollbar: false
				});
				wrapper2 = new iScroll('wrapper2', {
					disableMouse: true,
					disablePointer: true,
					vScrollbar: false
				});
				wrapper3 = new iScroll('wrapper3', {
					disableMouse: true,
					disablePointer: true,
					vScrollbar: false
				});
